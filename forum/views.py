...
class ForumExploreView(TemplateView):
    def get(self, request, *args, **kwargs):
        context=super(ForumExploreView,self).get_context_data(*args,**kwargs)
        context={'forum':True}
        if not request.user.is_authenticated:
            return render(request,'forum/forum_not_logged.html',context)
        else:
            ...
            return render(request,'forum/forum_explore.html', context)
